package vip.dtcloud.extractor.snmp.snmp;
...
import vip.dtcloud.extractor.snmp.info.DataTypes;
import vip.dtcloud.extractor.snmp.info.SnmpServiceReturnEnum;
import vip.dtcloud.extractor.snmp.info.SnmpServiceReturnMesage;
import vip.dtcloud.extractor.snmp.info.SnmpWalkResult;
import vip.dtcloud.extractor.snmp.util.SnmpUtil;
...
import static vip.dtcloud.extractor.snmp.service.WebServiceReturnEnum.FAILURE;
...
public class SnmpManagerImpl implements SnmpManager {
     ...
     * correct value.
     *
     * @param oId           Object identifier representing the functionality
     * @param communityName Community Name (by default public)
     * @param targetIP      IP address of target machine
     * @param portNumber    Port number (by default 161)
    ...
    public SnmpServiceReturnMesage get(final String oId, String communityName, final String targetIP, int portNumber) {
        ...
        if ((null == oId) || (null == targetIP) || (!SnmpUtil.isValidIp(targetIP)) || (!SnmpUtil.isValidOid(oId))) {
     ...
     * @param targetIP             IP address of target machine
     * @param portNumber           Port number (by default 161)
     * @param userName             the security name of the user
     * @param authenticatePassword the authentication password
     * @param privacyPassword      the privacy password
     * @return SnmpServiceReturnMesage represents the return value and status
     */
    @Override
            ...
            try {
                ...
                while (!isWalkComplete) {
                    ...
                    if (responsePDU == null) {
                        ...
                        SnmpWalkResult snmpWalkResult = new SnmpWalkResult(vb.getOid().toString(), vb.toValueString());
     ...
     *
     * @param oId           Object identifier representing the functionality to be set.
     * @param communityName Community Name (by default public)
     * @param targetIP      IP address of target machine
     * @param portNumber    Port number (by default 161)
     * @param value         Value to be set
     * @param type          type to the value to be set
     ...
     *
     * @param communityName Community name of the target
     * @param targetIP      IP address of Target machine
...
//        final Address address = new TcpAddress(inetAddress, portNumber);
        final OctetString community = new OctetString(communityName);
        // TODO modified.
        final TransportMapping transport = new DefaultUdpTransportMapping();
//        final TransportMapping transport = new DefaultTcpTransportMapping();
...
//        communityTarget.setVersion(SnmpConstants.version1);
     ...
     * @param isSNMPV3 True for SNMP Version 3 else false
     ...
     * @param type     The Type of the value to be set as per rfc2570
     *                 [INTEGER,COUNTER32
     ...
     */
    private PDU createSetPDU(final String oId, final String value, final String type, final boolean isSNMPV3) throws IOException {
        ...
        DataTypes setValueType = DataTypes.valueOf(type.toUpperCase());
        switch (setValueType) {
            case INTEGER:
     ...
     *
     * @param responseEvent ResponseEvent got from the snmp set/get operations
...
//                final Vector<VariableBinding> variableBindings = resultPdu.getVariableBindings();
                final List<? extends VariableBinding> variableBindings = resultPdu.getVariableBindings();
...
//                    final VariableBinding varBinding = variableBindings.firstElement();
                    final VariableBinding varBinding = variableBindings.get(0);
